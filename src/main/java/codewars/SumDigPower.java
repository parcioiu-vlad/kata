...
public class SumDigPower {
  ...
  public static List<Long> sumDigPow(long a, long b) {
    return LongStream.range(a, b).filter(SumDigPower::isValid).boxed().collect(Collectors.toList());
  }

  private static boolean isValid(long value) {
    String valueString = Long.toString(value);
    return IntStream.range(0, valueString.length())
            .mapToDouble(i -> Math.pow(Character.getNumericValue(valueString.charAt(i)), i + 1))
            .sum() == value;
  }
